...
SimpleNanoDevice::SimpleNanoDevice ()			//变量初始化
{
	...
	reSendTimeInterval = 0.5;			//flow-guided,发送节点重新发送检测邻居节点的时间间隔
	errorTime = 0.7;
...
void SimpleNanoDevice::HarvestEnergy() {
	...
	double nowCapEnergySpeed = random->GetValue(7.0, m_capEnergySpeed);			//根据随机数产生不同的能量捕获率,0.7-0.8pJ/s
	double HarEnergyThisTime = m_capEnergyInterval * nowCapEnergySpeed / 10;
...
double SimpleNanoDevice::GetMinSatisfidSendEnergy() {
	return (m_PacketSize) * m_EnergySendPerByte + (m_TestSize) * m_EnergyReceivePerByte;
}

double SimpleNanoDevice::TraditionalGetMinSatisfidForwardEnergy() {
	return (m_PacketSize) * m_EnergySendPerByte;
}

double SimpleNanoDevice::GetMinSatisfidForwardEnergy() {
...
void SimpleNanoDevice::EvendJudge() {
	if(nodeSchedule.IsExpired()) {
		...
		nodeSchedule = Simulator::Schedule (Seconds (nextTime), &MessageProcessUnit::CreateMessage, GetMessageProcessUnit());
	}
}
