	...
	if(GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		if (neighbors.size() != 0) {
			...
			if (ttl > 1) {
				...
				bool localFlag = false;				//邻居节点是否存在网关节点标志
				for (it = neighbors.begin(); it != neighbors.end(); it++) {
					std::cout << Simulator::Now().GetSeconds() << " " << "SendPacket--neighbor.id:" << (*it).detail_id << "   " << (*it).detail_phy->GetMobility()->GetPosition() << "    " << "packetId:" << p->GetUid() << "   " << "ttl:" << l3Header.GetTtl() << std::endl;
					if((*it).detail_type == 0)	{		//如果是网关节点,将数据包发送给网关节点，其它节点不接收
						localFlag = true;
						GetDevice()->GetMac()->Send(p);
					}
