...
#include "car-nano-routing-entity.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("CarNanoRoutingEntity");

namespace ns3 {


NS_OBJECT_ENSURE_REGISTERED (CarNanoRoutingEntity);

TypeId CarNanoRoutingEntity::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::CarNanoRoutingEntity").SetParent<Object>();
	return tid;
}


CarNanoRoutingEntity::CarNanoRoutingEntity ()
{
	SetDevice(0);
...
void  CarNanoRoutingEntity::DoDispose (void)
{
	SetDevice (0);
}

void CarNanoRoutingEntity::SendPacket (Ptr<Packet> p)
{
	if(GetDevice()->GetEnergyCapacity() >= GetDevice()->GetMinSatisfidSendEnergy()) {			//节点能量满足最小发送能量
		...
		//std::cout << Simulator::Now().GetFemtoSeconds() << "   Id:" << GetDevice()->GetNode()->GetId() << std::endl;
		...
		Simulator::Schedule (Seconds (GetDevice ()->reSendTimeInterval), &CarNanoRoutingEntity::SendPacket, this, p);			//0.1s
	}
}

void CarNanoRoutingEntity::SendGatewaytestPacket (Ptr<Packet> p)		//目的节点发送探测包,数据包添加序列头和路由头
{
	NanoL3Header l3Header;
	SenderTypeTag tag;
...
void CarNanoRoutingEntity::ReceivePacket (Ptr<Packet> p, Ptr<SpectrumPhy> sourcePhy)
{
	...
	if (GetDevice()->m_type == SimpleNanoDevice::NanoInterface) {				//如果是接收节点是网关节点
		...
		if(sourcePhy->GetDevice()->GetNode()->GetId() != 0) {			//发送节点是纳米节点
		...
		} else if(sourcePhy->GetDevice()->GetNode()->GetId() == 0) {		//发送节点是网关节点,纳米节点接收定位数据包
			GetDevice()->GetObject<SimpleNanoDevice>()->ConsumeEnergyReceive(GetDevice()->GetTestSize());		//纳米节点接收数据包消耗能量

			uint32_t packetTxTime = 11786800;			//传输一次144字节数据包的时间，数据包100字节，序列数据包12字节，路由头24字节，mac头8字节
			...
			double singalDivideDistance = GetDevice()->GetMessageProcessUnit()->txRangeNanoGateway / GetDevice()->GetMessageProcessUnit()->divideNum;
			uint32_t areaNum = distance / singalDivideDistance;
			uint32_t backoffTime = (packetTxTime + testTxTime + distanceTxTime) * 2 * areaNum;			//实验中传输时间不计算信道中传输的时间

			NanoSeqTsHeader seqTs;				//seqTs的size为12字节
			seqTs.SetSeq (GetDevice()->p->GetUid ());
			GetDevice()->p->AddHeader (seqTs);				//p->GetSize()为初始数据包大小+12

			//std::cout << Simulator::Now().GetFemtoSeconds() << "   Id:" << GetDevice()->GetNode()->GetId() << "   " << "singalDivideDistance:" << singalDivideDistance << "   " << "distance:" << distance << "   " << "areaNum:" << areaNum << "   " << "backoffTime:" << backoffTime << std::endl;
			Simulator::Schedule (FemtoSeconds(backoffTime), &CarNanoRoutingEntity::SendPacket, this, GetDevice()->p);
		}
	}
}

void CarNanoRoutingEntity::UpdateReceivedPacketId (uint32_t id)			//弹出一个最早的数据，push一个新数据
{
	m_receivedPacketList.pop_front ();				//pop一个又push一个，保持m_receivedPacketList的大小为20
	m_receivedPacketList.push_back (id);
}

bool CarNanoRoutingEntity::CheckAmongReceivedPacket (uint32_t id)
{
...
void CarNanoRoutingEntity::SetReceivedPacketListDim (int m) {
