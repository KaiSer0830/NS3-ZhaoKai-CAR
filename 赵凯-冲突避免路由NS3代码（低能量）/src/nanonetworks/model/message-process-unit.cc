...
void MessageProcessUnit::CreateMessage ()				//从主函数进入
{
	...
	if(GetDevice()->packetExistFlag == false && Simulator::Now().GetSeconds() < 10) {  //前一个数据包是否被接收完毕，一个节点只能拥有一个数据包，自己产生的或者是转发别人的
		...
		if(GetDevice()->GetObject<SimpleNanoDevice>()->GetL3()->GetObject<TraditionalNanoRoutingEntity>() != 0) {
...
void MessageProcessUnit::ProcessMessage (Ptr<Packet> p)				//用于冲突避免路由协议
{
	...
	uint32_t delay = Simulator::Now ().GetFemtoSeconds() - seqTs.GetTs().GetFemtoSeconds();			//seqTs的开始时间戳是在路由协议中的发送数据包函数确定的
//	std::cout << "delay:   " << delay << "   " << Simulator::Now ().GetFemtoSeconds() << "   " << seqTs.GetTs().GetFemtoSeconds() << "   " << Simulator::Now ().GetFemtoSeconds() - seqTs.GetTs().GetFemtoSeconds() << std::endl;
...
void MessageProcessUnit::ProcessMessage (Ptr<Packet> p, uint32_t backoffTime)				//用于传统定位路由协议与路由定位路由协议
{
	NanoL3Header l3Header;					//注意：需要按照堆栈的顺序来取，不能弄错，l3Header后放得先取
	p->RemoveHeader (l3Header);
	NanoSeqTsHeader seqTs;
	p->RemoveHeader (seqTs);				//会将原seqTs数据赋值到新创建的seqTs对象
	uint32_t delay = Simulator::Now ().GetFemtoSeconds() - seqTs.GetTs().GetFemtoSeconds() + backoffTime;			//seqTs的开始时间戳是在路由协议中的发送数据包函数确定的
	//std::cout << "delay:   " << delay << "   " << Simulator::Now ().GetFemtoSeconds() << "   " << seqTs.GetTs().GetFemtoSeconds() << "   " << Simulator::Now ().GetFemtoSeconds() - seqTs.GetTs().GetFemtoSeconds() << std::endl;
