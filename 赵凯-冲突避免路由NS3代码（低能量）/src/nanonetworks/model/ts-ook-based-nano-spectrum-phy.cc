...
bool TsOokBasedNanoSpectrumPhy::StartTx (Ptr<Packet> p)					//从FSANanoMacEntity::DoSendPacket中的phy->StartTx (p)过来
{
	...
	uint32_t duration = nbPulseIntervals * m_pulseInterval.GetFemtoSeconds () + m_pulseDuration.GetFemtoSeconds () * (nbPulseIntervals + 1);
	//std::cout << p->GetSize () << "   " << nbPulseIntervals << "   " << m_pulseInterval.GetFemtoSeconds () << "   " << m_pulseDuration.GetFemtoSeconds () << "   " << duration << std::endl;
	...
	Simulator::Schedule (FemtoSeconds (duration), &TsOokBasedNanoSpectrumPhy::EndTx, this, txParams);
	if (GetState () == IDLE) {
...
void TsOokBasedNanoSpectrumPhy::EndTx (Ptr<NanoSpectrumSignalParameters> txParams)
{
	...
	osPhyTx << GetDevice ()->GetNode ()->GetId () << " " << txParams->m_packet->GetUid () << std::endl;
...
void TsOokBasedNanoSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumParams)				//从NanoSpectrumChannel::StartTx(Ptr<SpectrumSignalParameters> txParams)中过来，接收数据的准备工作5
{
	...
	if (params != 0) {
		...
		NS_LOG_LOGIC(this << "state: " << GetState () << m_receivingpackets->size ());
		Simulator::Schedule(FemtoSeconds (duration), &TsOokBasedNanoSpectrumPhy::EndRx, this, params);
	}
}
...
uint32_t TsOokBasedNanoSpectrumPhy::GetTxDuration (Ptr<Packet> p)
{
