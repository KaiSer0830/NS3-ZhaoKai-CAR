...
#include "ns3/location-nano-routing-entity.h"
#include "ns3/traditional-nano-routing-entity.h"
#include "ns3/car-nano-routing-entity.h"
...
int main(int argc, char *argv[]) {
	...
	double txRangeNanoGateway = 0.012;			//纳米网关节点传输范围0.012m,确保在路由与网关信号能覆盖整个血管
	int seed = 1;
	...
	for(int circleNum = 1; circleNum < 2; circleNum++) {
		...
		for (int rouNum = 1; rouNum < 4; rouNum++) {
			...
			if (l3Type == 1) {
				routerHead = "traditional_";
			} else if (l3Type == 2) {
				routerHead = "location_";
			} else if (l3Type == 3) {
				routerHead = "car_";
			}
			for (int x = 1; x < 11; x++) {
...
void Run(int nbNanoNodes ,int nbNanoGateways, double txRangeNanoNodes,double txRangeNanoGateway, int macType, int l3Type, int seed, std::string postitionLogFile, std::string energyLogFile, std::string phyCollLogFile, std::string phyTxLogFile, std::string TxLogFile, std::string RxLogFile)
{
	std::cout << "START SIMULATION WITH: "
			...
			"\n\t macType [1->Fsa; 2->Fsa;3->Fsa] "<< macType <<
			"\n\t l3Type [1->Traditional Routing; 2->Location Routing; 3->Car Routing] "<< l3Type <<
			"\n\t seed " << seed << std::endl;
	...
	mobility.SetPositionAllocator ("ns3::RandomBoxPositionAllocator",
			"X", StringValue ("ns3::UniformRandomVariable[Min=0.012|Max=0.488]"),		//RandomVariableValue (UniformVariable (0, xrange)),
	...
	for (int x = 0; x < nbNanoGateways; x++) {
		...
		if (l3Type == 1) {
			Ptr<TraditionalNanoRoutingEntity> routing1 = CreateObject<TraditionalNanoRoutingEntity> ();
			routing = routing1;
		} else if (l3Type == 2) {
			Ptr<LocationNanoRoutingEntity> routing2 = CreateObject<LocationNanoRoutingEntity> ();
			routing = routing2;
		} else {
			Ptr<CarNanoRoutingEntity> routing3 = CreateObject<CarNanoRoutingEntity> ();
			routing = routing3;
		}
		...
		if (macType == 1) {
			Ptr<FSANanoMacEntity> mac1 = CreateObject<FSANanoMacEntity> ();
			mac = mac1;
		} else if (macType == 2) {
			Ptr<FSANanoMacEntity> mac2 = CreateObject<FSANanoMacEntity> ();
			mac = mac2;
		} else {
	...
	for (uint32_t i = 0; i < d_nodes.GetN(); i++) {
		...
		double energy = 0.8;					//存储能量0.8pJ,最小设置0.6pJ
		double maxenergy = 0.8;					//最大存储能量0.8pJ
		double capEnergyInterval = 0.1;			//能量捕获时隙0.1s
		double capEnergySpeed = 8.0;				//能量捕获速度0.7-0.8pJ/s
		double EnergySendPerByte = 8.0 / 2.0 * 100.0 * 4.0 / 1000000.0;					//发送能耗/字节	1.6*10^-3pJ/byte	计算公式：每个字节8位，平均一半1一半0，每个pulse100aJ，能量效率25%，
		double EnergyReceivePerByte = 8.0 / 2.0 * 100.0 * 4.0 / 1000000.0;		//接收能耗/字节	1.45*10^-3pJ/byte
		int PacketSize = 100.0;				//数据包大小100
		int testSize = 10;					//探测数据包大小30,这里的探测数据包是指纳米节点确定候选节点时发送的探测数据包
	...
	for (int i = 0; i < nbNanoNodes; i++) {
		...
		d_nodes.Get(i)->GetObject<SimpleNanoDevice>()->nodeSchedule = Simulator::Schedule(Seconds(startTime), &MessageProcessUnit::CreateMessage, mpu);		//每个纳米节点调用CreteMessage函数的间隔,间隔为0.2-0.3中一个随机时间
