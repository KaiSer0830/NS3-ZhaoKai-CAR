...
for i = 1:filesNum
    for j=1:nodeNum
        file1 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\traditional_Rx',num2str(j), '.log'];
        fid1 = fopen(file1);
        C1 = textscan(fid1, '%f%f%f%f%f%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid1);
        dataRx1 = [C1{7}];                         %%取出第7个元素
        file2 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\traditional_Tx', num2str(j), '.log'];
        fid2 = fopen(file2);
...
for i = 1:filesNum
    for j=1:nodeNum
        file3 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\location_Rx',num2str(j), '.log'];
        fid3 = fopen(file3);
        C3 = textscan(fid3, '%f%f%f%f%f%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid3);
        dataRx2 = [C3{7}];                         %%取出第7个元素
        file4 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\location_Tx', num2str(j), '.log'];
        fid4 = fopen(file4);
...
for i = 1:filesNum
    for j=1:nodeNum
        file5 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\car_Rx',num2str(j), '.log'];
        fid5 = fopen(file5);
        C5 = textscan(fid5, '%f%f%f%f%f%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid5);
        dataRx3 = [C5{7}];                         %%取出第7个元素
        file6 = ['D:\professional_install\matlab\bin\project_files\conflict_zhao\log', num2str(i),'\car_Tx', num2str(j), '.log'];
        fid6 = fopen(file6);
...
legend('TFGR', 'RLR', 'CAR','Location','southeast');
